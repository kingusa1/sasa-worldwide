...
export const SASA_KNOWLEDGE = {
  ...
  services: {
    b2c: {
      ...
      description: "Elite field teams executing door-to-door, residential, and retail campaigns with precision and professionalism.",
      features: [
    ...
    b2b: {
      ...
      description: "High-performance corporate activation programs for enterprise clients.",
      features: [
  ...
  technology: {
    platform: {
      ...
      features: ["Real-time dashboards", "Performance tracking", "KPI monitoring", "AI-enabled oversight"],
    },
    academy: {
  ...
  csr: {
    overview: "SASA is committed to making a positive impact in the UAE community.",
    initiatives: [
      { name: "Youth Employment Program", impact: "500+ youth employed annually" },
      { name: "Sustainable Operations", impact: "40% paper reduction" },
      { name: "Skills Development", impact: "1,000+ trained annually" },
      { name: "Community Outreach", impact: "50+ events yearly" },
      { name: "Emiratization Support", impact: "30% workforce target" },
      { name: "Digital Inclusion", impact: "200+ workshops" },
    ],
  },

  partnerships: {
    overview: "SASA partners with leading brands across industries to deliver exceptional sales results.",
    featured: "SASA × CAFU Fuel Subscription partnership",
    benefits: [
...
SERVICES:
1. B2C Residential - ${k.services.b2c.description}
2. B2B Corporate - ${k.services.b2b.description}
3. B2B2C Hybrid - ${k.services.b2b2c.description}
4. B2G Government - ${k.services.b2g.description}

CAREERS (5-Phase Path):
${k.careers.phases.map(p => `- Phase ${p.phase}: ${p.title} (${p.duration})`).join('\n')}

TECHNOLOGY:
- SASA OS: ${k.technology.platform.description}
- SASA AI Academy: ${k.technology.academy.description}

VALUES: ${k.values.map(v => v.name).join(', ')}
